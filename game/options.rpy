## 本opotions.rpy仅包含《现在就想告诉哥哥我是妹妹！》的相关选项代码，其余与此游戏配置无关的代码均已删除。
## This file only contains the relative commands of "Ima sugu oniichan ni imouto datte iitai"(いますぐお兄ちゃんに妹だっていいたい！),
## any command line which has nothing to do with this game has been deleted.
...
init -1 python hide:
    ...
    config.window_title = u"いますぐお兄ちゃんに妹だっていいたい！ Ren'py"
    ...
    theme.tv(
        ...
        mm_root = "sysgrp/SGTitle990000.grp.png",
    ...
    style.button.activate_sound = "system/KC017.ogg"
    style.imagemap.activate_sound = "system/KC017.ogg"
...
python early:
    config.save_directory = "ima_imo-1452922647"
...
init -1 python hide:
    ...
    ## Windows 用标题栏图标设定。
    ## 图像尺寸必须为 32x32 。
    config.windows_icon = "system/head.ico"
    ...
    config.hard_rollback_limit = 10000
...
init python:
    ...
    build.directory_name = "ima_imo_Ren'py"

    ## 游戏可执行程序的文件名。
    ## 例如设置为“mygame”时，则生成发行版后，
    ## 其游戏目录下可执行文件的文件名将会为“mygame.exe”，
    build.executable_name = u"いますぐお兄ちゃんに妹だっていいたい！"
    ...
    build.archive('sysgrp', 'all')
    build.archive('cg', 'all')
    build.archive('movie', 'all')
    ...
    build.classify('game/cg1/*.png', 'cg')
